...
Each zone is two cards: **OFF + sources** and **volume + mute**. The right-hand rail holds the room-combination button (opens the partition page) and wall chips. That page shows **global** wall state from Logic `Wall_AB_Open_FB` / `Wall_BC_Open_FB` (fan those to every XPanel). Toggles invert those FBs. Each visible wall has a Combine / Divide toggle (`Wall_AB_Toggle` / `Wall_BC_Toggle`) on every panel that can see that wall. When A+B+C, Room A must close B\|C before A\|B; Room C must close A\|B before B\|C. Combine all / divide all (`Wall_Combine_All` / `Wall_Divide_All`) is shown only when that panel’s `Master_Mode` is high. Power-off uses [Power Shutdown Confirmation v1.0](https://github.com/Prophet6/Crestron-Modules/tree/main/power-shutdown-confirmation) (one instance per panel).
